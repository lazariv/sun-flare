...
summarisedTOT %>% na.omit() %>% 
  ...
  ggplot(aes(x=mean, y=skewness, colour=LABEL)) + geom_point(size=0.2) + 
  #ggplot(aes(x=mean/sd, colour=LABEL)) + stat_ecdf() + 
  facet_wrap(LABEL~.)



variable = c("EPSX", "MEANALP")
summarisedTOT = data %>% 
  mutate(ID = as.factor(ID), LABEL = as.factor(LABEL), TIME=as.factor(TIME)) %>% 
  select(variable, TIME, ID, LABEL) %>% 
  group_by(ID, LABEL) %>% 
  summarise_if(is.numeric, list(~mean(., na.rm=TRUE), ~sd(., na.rm=TRUE), ~skewness(., na.rm=TRUE)), na.rm=TRUE) 
summarisedTOT %>% na.omit() %>%
  mutate(EPSX_ratio = EPSX_mean/EPSX_sd, MEANALP_ratio = MEANALP_mean/MEANALP_sd) %>% 
  #  filter(TOTBSQ > 0 & TOTBSQ < 5e10) %>% 
  ggplot(aes(x=EPSX_ratio, y=MEANALP_ratio, colour=LABEL)) + geom_point(size=0.2) + 
  #ggplot(aes(x=mean/sd, colour=LABEL)) + stat_ecdf() + 
  facet_wrap(LABEL~.)

summarisedTOT %>% na.omit() %>%
  mutate(mean_ratio = EPSX_mean/MEANALP_mean, sd_ratio = EPSX_sd/MEANALP_sd) %>% 
  ggplot(aes(x=mean_ratio, y=sd_ratio, colour=LABEL)) + geom_point(size=0.2)
