...
model1 %>% 
  layer_conv_1d(kernel_size=7, filters=64, activation="relu", input_shape=list(25*60, 1)) %>% 
...
model1 %>% save_model_hdf5("my_conv1d_model_with_more_params.h5")
...
write.csv(results, file="submission4_more_params.csv", row.names=FALSE)
